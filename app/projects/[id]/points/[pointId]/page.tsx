...
export default async function PointDetailPage({
  ...
  const hasPhotos = Boolean(point.sample_photo_url || point.outcrop_photo_url);
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      <div className="mx-auto max-w-7xl p-6">
        <div className="mb-6 flex flex-wrap gap-3 text-sm">
          <Link href={`/projects/${id}/points`} className="text-slate-300 underline">
            ← Back to Points
          </Link>

          <Link href={`/projects/${id}/map`} className="text-slate-300 underline">
            Back to Map
          </Link>
          ...
          <div className="mb-5 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Sample and outcrop photos for field verification
              </p>
            </div>

            <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-700">
              {(point.sample_photo_url ? 1 : 0) +
                (point.outcrop_photo_url ? 1 : 0)}{" "}
              photos
            </span>
          </div>
